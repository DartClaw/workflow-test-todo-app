...
class TestTodos:
    ...
    def test_delete_todo(self, authenticated_client, test_todo, db_session):
        """Test deleting a todo and updating the sidebar count via OOB swap."""
        remaining = Todo(
            list_id=test_todo.list_id,
            title="Remaining Todo",
            position=1,
        )
        another_remaining = Todo(
            list_id=test_todo.list_id,
            title="Another Remaining Todo",
            position=2,
        )
        completed = Todo(
            list_id=test_todo.list_id,
        ...
        db_session.add_all([remaining, another_remaining, completed])
        db_session.commit()
    ...
    def test_delete_last_incomplete_todo_returns_zero_count(
        self, authenticated_client, test_todo, db_session
    ):
        """Test deleting the last incomplete todo returns a zero count."""
