...
- [x] **S01 [OC01,OC02] [TI01,TI02] Edit-dialog save persists and round-trips a Due Date**
...
- [x] **S02 [OC02] [TI02,TI03] Reopening the saved Todo pre-fills the same date**
...
- [x] **S03 [OC03] [TI01,TI04] Empty Due Date input clears an existing stored date**
...
- [x] **S04 [OC03] [TI01,TI04] Malformed non-dialog date input does not overwrite the stored date**
...
- [x] The `PUT /api/todos/{todo_id}` edit flow keeps the current HTMX row-swap contract by returning the todo row partial shape expected by `hx-target="#todo-{id}"`.
- [x] BUG-002 regression coverage is owned in the todo update test surface and does not require shared quick-add or priority test changes.
- [x] Persisted Due Date values continue to round-trip through the existing `format_date_input()` and visible row-display formatting contracts.
...
- [x] **TI01 Due Date edits use one supported input contract across save, clear, and malformed-input paths**
...
- [x] **TI02 Rendered todo rows carry the persisted Due Date through visible text and reopen data**
...
- [x] **TI03 The existing HTMX edit response contract remains row-swap compatible**
...
- [x] **TI04 BUG-002 regression coverage proves the story without touching BUG-003 surfaces**
