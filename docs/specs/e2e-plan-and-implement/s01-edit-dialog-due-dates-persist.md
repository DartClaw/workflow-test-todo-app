...
- [x] **S01 [OC01] [TI01,TI03] Edit-dialog date saves and reopens with the same value**
...
- [x] **S02 [OC02] [TI01,TI03] Clearing the dialog date removes the stored due date**
...
- [x] **S03 [OC03] [TI02,TI03] Malformed due-date payloads fail explicitly**
  - **Given** an existing `Todo` whose `due_date` is already set
...
- [x] The edit todo route continues returning HTML partials and `partials/error.html` rather than JSON validation errors.
- [x] The due-date round-trip stays aligned with the app's existing `YYYY-MM-DD` input/output contract used by `format_date_input`.
- [x] Existing title, priority, and authorization behavior on `/api/todos/{todo_id}` remains unchanged.
...
- [x] **TI01** Edit todo saves the date format emitted by the current dialog and preserves it for reopen
...
- [x] **TI02** Malformed edit-dialog due-date payloads fail visibly instead of disappearing
...
- [x] **TI03** Todo update regression coverage proves set, clear, and malformed-date behavior at the route boundary
